...
from nndef_iterdist_gru2resnet import GRUResNet
...
def main():
    ...
    coords = []
    with open(sys.argv[2], 'r') as refpdbfile:
    ...
    init_dmap = torch.from_numpy(init_dmap).type(torch.FloatTensor).contiguous().to(device)
    ...
    with open(sys.argv[3] + ".hb", "w") as f:
    ...
    with open(sys.argv[3] + ".dist", "w") as f:
    ...
    with open(sys.argv[3] + ".phi", "w") as f:
        for wi in range(length):
    ...
    with open(sys.argv[3] + ".psi", "w") as f:
        for wi in range(length):
            for wj in range(wi+1, length):
