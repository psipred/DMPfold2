...
int main(int argc, char **argv)
{
    ...
    if (argc != 2)
	fail("usage : qmodope_mainens ensemble.pdb");
	...
	{
	    fprintf(ofp, "ATOM  %5d %s %s  %4d    %8.3f%8.3f%8.3f%6.2f%6.2f\n",
	...
	sprintf(cmdstr, "python dope.scr qmod_temp.pdb | tail -1");
	...
	sprintf(cmdstr, "./qmodcheck qmod_temp.pdb | tail -1 > qmod.tmp"); // Modified by Joe to add "./"
//	sprintf(cmdstr, "./qmodcheck qmod_temp.pdb A | tail -1");
	...
	if (fscanf(qfp, "%f", &qm_val) == 1)
	{
	    ...
	    {
		best_qm = qm_val;
		system("/bin/cp -f qmod_temp.pdb best_qdope.pdb");
	    }
	}
	else
	    puts("FAILED!");
