...
def modeller_fa_and_score(env, iter_n, n_trajs, output, sequence):
    n_res = output.size(2)
    dope_scores = []
    ...
    with open(ali_fp, "w") as of:
        ...
        of.write(f"sequence:target:1:A:{n_res}:A:x:x:2.0:-1.0\n")
        of.write(f"{sequence}*\n")
        for i in range(n_trajs):
            of.write("\n")
            of.write(f">P1;traj_{i + 1}\n")
            of.write(f"structureX:traj_{i + 1}:1:A:{n_res}:A:x:x:2.0:0.02\n")
            of.write(f"{sequence}*\n")
...
def aln_to_model_fdf(aln_filepath, out_dir, ncycles=-1, nmodels1=-1, nmodels2=-1):
    if ncycles == -1:
        ncycles = 3
    if nmodels1 == -1:
        nmodels1 = 20
    ...
    for iter_n in range(2, ncycles + 1):
        print(f"Starting iteration {iter_n} of {ncycles}")
        print()
